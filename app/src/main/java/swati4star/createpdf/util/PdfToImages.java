...
public class PdfToImages extends AsyncTask<Void, Void, Void> {
    ...
    protected Void doInBackground(Void... voids) {
        ...
        try {
            fileDescriptor = getParcelFileDescriptor(fileDescriptor);
            if (fileDescriptor != null) {
                PdfRenderer renderer = new PdfRenderer(fileDescriptor);
                final int pageCount = renderer.getPageCount();
                renderPages(renderer, pageCount);
    ...
    private ParcelFileDescriptor getParcelFileDescriptor(ParcelFileDescriptor fileDescriptor) throws FileNotFoundException {
    ...
    private void renderPages(PdfRenderer renderer, int pageCount) {
        for (int i = 0; i < pageCount; i++) {
            Bitmap bitmap = getBitmap(renderer, i);
    ...
    private Bitmap getBitmap(PdfRenderer renderer, int i) {
